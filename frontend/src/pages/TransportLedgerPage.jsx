...
export default function TransportLedgerPage() {
  ...
  return <div className="page-shell ledger-page">
    ...
    <section className="card-clean ledger-panel"><div className="ledger-heading"><span className="ledger-heading-icon"><Truck size={19}/></span><div><h2>สรุปยอดสุทธิต่อทะเบียน</h2><p>ประจำเดือน {month} · กำไรเที่ยวงาน + รายรับอื่น − รายจ่ายที่กรอกเอง</p></div></div><div className="ledger-table-scroll"><table className="ledger-table"><thead><tr><th>ทะเบียนรถ</th><th>กำไรจากบิล + รายรับอื่น</th><th>รายจ่ายที่กรอกเอง</th><th>คงเหลือ / กำไร(ขาดทุน)</th><th>รายการ</th></tr></thead><tbody>{(data?.by_vehicle || []).filter(v=>selectedVehicle==='all'||v.vehicle_id===selectedVehicle).map(v=><tr key={v.vehicle_id}><td><strong>{v.plate_no}</strong></td><td>{format(v.income_satang)} ฿</td><td>{format(v.expense_satang)} ฿</td><td className={v.profit_satang<0?'ledger-negative':'ledger-positive'}>{format(v.profit_satang)} ฿</td><td>{v.entries}</td></tr>)}</tbody></table></div></section>
    <section className="card-clean ledger-panel"><div className="ledger-heading"><span className="ledger-heading-icon"><CalendarDays size={19}/></span><div><h2>รายการกำไรเที่ยวงานและรับ–จ่ายย้อนหลัง</h2><p>{rows.length} รายการ · บิลเชื่อมอัตโนมัติแก้ที่บันทึกงาน · รายการที่กรอกเองแก้ไขหรือลบได้</p></div></div><CompactPager state={pager} label="ค้นหาวันที่ ทะเบียน วัสดุ หรือหมวดบัญชี"/><div className="ledger-table-scroll"><table className="ledger-table"><thead><tr><th>วันที่</th><th>ทะเบียนรถ</th><th>ประเภท / หมวด</th><th>วัสดุที่ขน / จำนวน</th><th>จำนวนเงิน</th><th>หมายเหตุ</th><th>จัดการ</th></tr></thead><tbody>{pager.visible.map(r=><tr key={r.id}><td>{r.date}</td><td><strong>{r.plate_no}</strong></td><td><span className={`ledger-pill ${r.type}`}>{r.is_auto?'กำไรจากบิล':r.type==='income'?'รายรับอื่น':'รายจ่าย'}</span><div className="ledger-cell-sub">{r.category}</div></td><td>{r.material||'-'}<div className="ledger-cell-sub">{r.quantity||''}</div></td><td className={r.type==='income'?'ledger-positive':'ledger-negative'}>{r.excluded_from_totals?'ไม่คิดซ้ำ · ':r.type==='income'?'+':'−'}{format(r.amount_satang)} ฿</td><td>{r.note||'-'}{r.excluded_from_totals&&<div className="ledger-cell-sub">ยืนยันรายการซ้ำกับบิลแล้ว · ไม่รวมยอด · บิล {r.matched_delivery_id}</div>}{possibleDuplicateIds.has(r.id)&&<div className="ledger-cell-sub">อาจซ้ำ กรุณาตรวจสอบ</div>}{r.is_auto&&<div className="ledger-cell-sub">รายรับเที่ยว {format(r.trip_income_satang)} · ต้นทุนในบิล {format(r.trip_cost_satang)} บาท</div>}</td><td>{r.is_auto?<span className="ledger-auto-tag">เชื่อมจากบิล</span>:<div className="ledger-row-actions"><button type="button" title="แก้ไข" onClick={()=>edit(r)}><Pencil size={15}/></button><button type="button" title="ลบ" onClick={()=>remove(r)}><Trash2 size={15}/></button></div>}</td></tr>)}{!rows.length&&<tr><td colSpan={7} className="ledger-empty">{loading?'กำลังโหลด...':'ยังไม่มีรายการในเดือนที่เลือก'}</td></tr>}</tbody></table></div></section>
  </div>;
}
